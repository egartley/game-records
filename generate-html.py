...
def get_playtext(game):
    # 0 means incomplete, don't display playthrough count if only 1
...
def get_listing_html(game):
    # returns individual listing html (gamecard)
...
def calc_stats():
    for game in game_list:
        ...
        # platform
        platform_index = 6
        if game.platform in ["PlayStation", "PlayStation 4", "PlayStation 5"]:
            platform_index = 0
...
if "at 0" in datestring:
    # correct formatting for times with a single digit hour
